...
const PYTHON_CRYPTO_PACKAGES: &[(&str, &str)] = &[
    ("cryptography", "General-purpose cryptographic library (hazmat primitives, X.509, etc.)"),
    ("pycryptodome", "PyCryptodome: AES, RSA, DES, hashing, and more"),
    ("pycryptodomex", "PyCryptodome (alternative namespace)"),
    ("pynacl", "Python binding to libsodium (Curve25519, Ed25519, ChaCha20)"),
    ...
    ("paramiko", "SSH2 protocol library (RSA, ECDSA, AES, ChaCha20)"),
    ("bcrypt", "bcrypt password hashing"),
    ("passlib", "Password hashing framework (bcrypt, scrypt, argon2, etc.)"),
    ("argon2-cffi", "Argon2 password hashing"),
    ("itsdangerous", "Data signing with HMAC (Flask sessions, etc.)"),
...
const NPM_CRYPTO_PACKAGES: &[(&str, &str)] = &[
    ("jsonwebtoken", "JWT signing/verification (HMAC, RSA, ECDSA)"),
    ("bcrypt", "bcrypt password hashing"),
    ("bcryptjs", "Pure-JS bcrypt implementation"),
    ("crypto-js", "JavaScript cryptographic library (AES, DES, SHA, HMAC)"),
    ...
    ("express-session", "Session management (may use crypto for session IDs)"),
    ...
    ("elliptic", "Elliptic curve cryptography (secp256k1, P-256, etc.)"),
...
const CARGO_CRYPTO_PACKAGES: &[(&str, &str)] = &[
    ...
    ("ring", "Cryptographic primitives (AES, SHA, RSA, ECDSA, Ed25519)"),
...
/// Parse `package-lock.json` for known crypto npm packages.
fn parse_package_lock_json(contents: &str, file_path: &str, findings: &mut Vec<CryptoAsset>) {
    ...
    if let Some(packages) = parsed.get("packages").and_then(|p| p.as_object()) {
        for (pkg_path, _) in packages {
            // Keys look like "node_modules/jsonwebtoken"
...
fn check_npm_package(pkg_name: &str, file_path: &str, findings: &mut Vec<CryptoAsset>) {
        ...
        .find(|(name, _)| *name == pkg_name)
    {
        ...
        if findings.iter().any(|f| {
            f.library_source == pkg_name && f.file_path == file_path
        }) {
            return;
        }
        let mut asset = CryptoAsset::new(
...
/// Parse `Cargo.lock` for known crypto Rust crates.
fn parse_cargo_lock(contents: &str, file_path: &str, findings: &mut Vec<CryptoAsset>) {
    // Cargo.lock uses TOML format with [[package]] sections
    // We do lightweight parsing without pulling in a TOML crate
    ...
    for line in contents.lines() {
        ...
        if line == "[[package]]" {
            ...
            if let Some(ref name) = current_name {
                check_cargo_package(name, file_path, findings);
            }
            current_name = None;
        }
...
mod tests {
    ...
    fn test_extract_python_package_name() {
        assert_eq!(extract_python_package_name("cryptography==41.0.0"), "cryptography");
        ...
        assert_eq!(extract_python_package_name("argon2-cffi[dev]"), "argon2_cffi");
